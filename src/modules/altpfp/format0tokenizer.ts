...
export default class Tokenizer {
  protected static readBufferToken(
    ...
    if (
      pixels.findNext((colour) => !colourEqualTo(palette[0])(colour)) === null
    ) {
      pixels.finish();
      return null;
    }
    if (pixels.offset < OffsetColourLong.maxOffset) {
      ...
      if (
        peek &&
        colourDifference(peek, palette[0]) < MAX_BACKGROUND_APPROXIMATION()
      ) {
        pixels.skip();
        return null;
      }
  ...
  static selectPalette(image: string[]): RGB[] {
    ...
    while (
      palette.length < PaletteSpecifier.expectedLength &&
  ...
  static tokenStatistics(tokens: ImageContentToken[], metadata: ImageMetadata) {
    ...
    if (tokenRatio > 0.5) console.warn(warning);
...
of OffsetColour tokens are of subtype OFFSET and OFFSET_LONG`);
    ...
    const zeroWarning = `${percent(
      zeroRatio
    )} of OffsetColour tokens have an offset of zero`;
    if (zeroRatio > 0.5) console.warn(zeroWarning);
    else console.info(zeroWarning);

    console.info(`${percent(maxRatio)} and ${percent(
      maxRatioLong
    )} of OFFSET and OFFSET_LONG tokens \
have their maximum respective offset`);
    ...
    const bgPaletteWarning = `${percent(
      bgPaletteRatio
    )} of tokens are background PaletteSelection tokens`;
